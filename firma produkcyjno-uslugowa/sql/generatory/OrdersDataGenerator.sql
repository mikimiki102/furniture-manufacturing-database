...
DECLARE @PctOldToCanceled int = 12;  
DECLARE @PctCurrToStatus2 int = 10;  
...
E AS (
  SELECT EmployeeID, ROW_NUMBER() OVER (ORDER BY EmployeeID) rn
  FROM dbo.Employees
  WHERE isActive = 1
),
...
S AS (
  SELECT ShipperID, ROW_NUMBER() OVER (ORDER BY ShipperID) rn
  FROM dbo.Shippers
  WHERE isActive = 1
),
...
G AS (
  SELECT
    ...
    (SELECT CustomerID FROM C CROSS JOIN CC WHERE C.rn=(ABS(CHECKSUM(t.rn,'C'))%CC.c)+1) AS CustomerID,
    (SELECT EmployeeID FROM E CROSS JOIN EC WHERE E.rn=(ABS(CHECKSUM(t.rn,'E'))%EC.c)+1) AS EmployeeID,
    (SELECT ShipperID  FROM S CROSS JOIN SC WHERE S.rn=(ABS(CHECKSUM(t.rn,'S'))%SC.c)+1) AS ShippersID,

    (SELECT Country FROM @Countries WHERE idx=(ABS(CHECKSUM(t.rn,'K'))%@CntCountries)+1) AS Country,
    (SELECT City    FROM @Cities    WHERE idx=(ABS(CHECKSUM(t.rn,'M'))%@CntCities)+1)    AS City,
    (SELECT Street  FROM @Streets   WHERE idx=(ABS(CHECKSUM(t.rn,'U'))%@CntStreets)+1)   AS Street,

    1+(ABS(CHECKSUM(t.rn,'H'))%220) AS HouseNo,
    CASE WHEN ABS(CHECKSUM(t.rn,'A'))%4=0 THEN 1+(ABS(CHECKSUM(t.rn,'B'))%80) END AS AptNo,

    CAST((ABS(CHECKSUM(t.rn,'P'))%8000)/100.0 AS decimal(10,2)) AS DeliveryPrice
  FROM T t
),
...
Curr AS (
  SELECT TOP (30)
    *,
    DATEADD(day,-ABS(CHECKSUM(rn,'D'))%5,@Today) AS OrderDate,
    CAST(NULL AS date) AS DeliveryDate,
    CASE
      WHEN (ABS(CHECKSUM(rn,'CUR_S2')) % 100) < @PctCurrToStatus2 THEN 2
      ELSE 3
    END AS Status
  FROM G
  ORDER BY rn
...
Old AS (
  SELECT TOP (470)
    *,
    DATEADD(day,-(5+ABS(CHECKSUM(rn,'O'))%176),@Today) AS OrderDate,
    CASE
      WHEN (ABS(CHECKSUM(rn,'OLD_CAN')) % 100) < @PctOldToCanceled THEN CAST(NULL AS date)
      ELSE DATEADD(day,5+ABS(CHECKSUM(rn,'L'))%5,
                   DATEADD(day,-(5+ABS(CHECKSUM(rn,'O'))%176),@Today))
    END AS DeliveryDate,
    CASE
      WHEN (ABS(CHECKSUM(rn,'OLD_CAN')) % 100) < @PctOldToCanceled THEN 1
      ELSE 4
    END AS Status
  FROM G
  ORDER BY rn
...
SELECT
  ...
  N'Ul. '+Street+N' '+CAST(HouseNo AS nvarchar(10))+ISNULL(N'/'+CAST(AptNo AS nvarchar(10)),N'') AS Address,
